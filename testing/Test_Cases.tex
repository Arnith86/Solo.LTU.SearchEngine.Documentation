...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
        \item Document A contains only "cat".
        \item Document B contains only "dog".
        \item Document C contains both "cat" and "dog".
    \end{itemize}
  \end{itemize}
\end{itemize}
...
Specifically testing term which stem to same character sequence. 
...
\begin{itemize}
  ...
  \item[] \textit{Example setup:}
  \begin{itemize}
      \item Document A contains the word "test" assigned the language "sv".
      \item Document B contains the word "hello" assigned the language "en".
      \item Document C contains the word "test" assigned the language "en" 
  \end{itemize}
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
        \item Enter the single term \texttt{hello} language \texttt{en} into the search input.
        \item Execute the search.
        \item Clear results and enter the term \texttt{test} language \texttt{en}.
        \item Execute the search.
    \end{enumerate}
...
\begin{itemize}
        ...
        \item For the query \texttt{test}: Both Document A and C are returned.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
      \item Document A contains the exact phrase ``fat cat'' and the word ``dog''.
      \item Document B contains the exact phrase ``fat cat'' but does \textbf{not} contain the word ``dog''.
      \item Document C contains the word ``dog'' but does \textbf{not} contain the exact phrase ``fat cat''.
      \item Document D contains neither the phrase ``fat cat'' nor the word ``dog'', but contains the term ``cat''.
  \end{itemize}
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
 \item Enter the query \texttt{+"fat cats" OR dog} into the search input.
  \item Execute the search.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item Only documents containing the exact phrase ``fat cat'' are returned.
...
\section{Test Case: TC-FRQ-3008}
\subsection*{Related Requirements}
FRQ-3008

\subsection*{Description}
Verify that the search engine correctly implements exclusion logic using both the keyword \textbf{NOT}
(case-sensitive) and the symbols \textbf{!} or \textbf{-}. The test also ensures that the system 
enforces the constraint that an exclusion operator must be preceded by a positive term, preventing 
standalone exclusion queries from returning results.

\subsection*{Preconditions}
\begin{itemize}
  \item The search system is operational and the index is populated with documents containing various combinations of terms (e.g., some with "cat", some with "dog", and some with both).
  \item The query parser is configured to recognize \texttt{NOT}, \texttt{!}, and \texttt{-} as exclusion operators.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Keyword Exclusion:} Execute a query using the \texttt{NOT} operator (e.g., \texttt{"cat NOT dog"}).
  \item \textbf{Symbol Exclusion:} Execute queries using the symbols (e.g., \texttt{"cat -dog"} and \texttt{"cat !dog"}).
  \item \textbf{Standalone Constraint:} Execute a standalone exclusion query (e.g., \texttt{"-dog"} or \texttt{"NOT dog"}).
  \item \textbf{Case Sensitivity:} Execute a query with a lowercase exclusion (e.g., \texttt{"cat not dog"}).
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item \textbf{Logical Filtering:} The queries \texttt{"cat NOT dog"}, \texttt{"cat -dog"}, and \texttt{"cat !dog"} all return documents containing "cat" but strictly exclude any documents that also contain "dog".
  \item \textbf{Constraint Enforcement:} Standalone exclusion queries (e.g., \texttt{"-dog"}) return either an explicit error message or zero results, satisfying the requirement for a preceding positive term.
  \item \textbf{Case Sensitivity:} The lowercase \texttt{"not"} is treated as a literal search term and does not perform an exclusion.
  \item \textbf{Consistency:} All three supported exclusion syntaxes (\texttt{NOT}, \texttt{!}, \texttt{-}) yield identical result sets when used with the same terms.
\end{itemize}
...
% ==========================================================
\section{Test Case: TC-FRQ-3012}
\subsection*{Related Requirements}
FRQ-3012

\subsection*{Description}
Verify that the search engine correctly implements pagination for query results. The test ensures that 
when a search query yields more than 10 results, the system divides the output into pages, returning only 
10 results per request and providing a mechanism to access subsequent pages.

\subsection*{Preconditions}
\begin{itemize}
  \item The search service is connected to a mock database or an in-memory index.
  \item The index is populated with at least 15 distinct documents that all match a specific test term (e.g., "LTU").
  \item The API or search controller is accessible for unit or integration testing.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Initial Request:} Execute a search query for the term "LTU" without specifying a page index (defaulting to page 1).
  \item \textbf{Inspect Count:} Count the number of document references returned in the first result set.
  \item \textbf{Second Request:} Execute the same search query but specify the second page (e.g., \texttt{page=2} or \texttt{offset=10}).
  \item \textbf{Inspect Continuity:} Compare the results from page 1 and page 2 to ensure no duplicate documents exist between the pages.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item \textbf{Page 1 Limit:} The first request returns exactly 10 results, even though 15 matches exist in total.
  \item \textbf{Page 2 Remainder:} The second request returns the remaining 5 results.
  \item \textbf{Exclusivity:} There is zero overlap between the results of page 1 and page 2.
  \item \textbf{Metadata:} The response object contains pagination metadata (e.g., \textit{TotalResults: 15}, \textit{CurrentPage: 1}, \textit{TotalPages: 2}).
\end{itemize}

% ==========================================================
% ==========================================================
\section{Test Case: TC-FRQ-3013}
\subsection*{Related Requirements}
FRQ-3013

\subsection*{Description}
Verify that each search result returned by the query engine includes the relevant headline 
(title or primary header) of the source document where the match was found. This ensures that the user 
receives enough context to identify the page before clicking the link.

\subsection*{Preconditions}
\begin{itemize}
  \item The search index is populated with a document having a known title (e.g., "Student Services at LTU") and body content.
  \item The search service is isolated for testing, or an integration test is set up against a mock database.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Search Execution:} Execute a search query for a term known to exist in the body content of the target document (e.g., "scholarships").
  \item \textbf{Intercept Result:} Capture the list of search result objects returned by the \textit{SearchService} or API.
  \item \textbf{Inspect Metadata:} Examine the properties of the first result item in the list.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item \textbf{Presence:} The result object contains a \texttt{Headline} or \texttt{Title} field.
  \item \textbf{Accuracy:} The content of the headline field exactly matches the title stored in the database for that document.
  \item \textbf{Expected Value:} The result for the query "scholarships" displays the headline: \texttt{"Student Services at LTU"}.
  \item \textbf{Completeness:} Every result in the returned set (up to the pagination limit) includes its respective headline.
\end{itemize}

% ==========================================================
% ==========================================================
\section{Test Case: TC-FRQ-3014}
\subsection*{Related Requirements}
FRQ-3014 (Snippet Generation and Highlighting)

\subsection*{Description}
Verify that search results include a contextual snippet of the page content where the search 
terms are found. The test also ensures that all occurrences of the search keywords within this snippet are 
visually distinguished (highlighted) using standard HTML tags (e.g., \texttt{<mark>} or \texttt{<b>}).

\subsection*{Preconditions}
\begin{itemize}
  \item The search index contains a document with a specific body text (e.g., "The search engine at LTU provides fast results for students.").
  \item The search service is configured to generate snippets with a predefined maximum length.
  \item The highlighting logic is integrated into the search result transformation layer.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Search Execution:} Execute a search query for the term "students".
  \item \textbf{Intercept Result:} Capture the search result object for the target document.
  \item \textbf{Inspect Snippet:} Examine the \texttt{Snippet} field of the returned result.
  \item \textbf{Verify Highlighting:} Search for the presence of highlighting tags around the keyword "students" within the snippet string.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item \textbf{Contextual Snippet:} The result contains a non-empty snippet string derived from the document's body content.
  \item \textbf{Highlighting Tags:} The keyword is wrapped in the expected tag.
  \item \textbf{Expected Value:} \texttt{... fast results for students.}
  \item \textbf{Sanitization:} The rest of the snippet is correctly encoded to prevent XSS, while the highlighting tags are preserved.
\end{itemize}

% ==========================================================
% ==========================================================
\section{Test Case: TC-FRQ-3015}
\subsection*{Related Requirements}
FRQ-3015

\subsection*{Description}
Verify that the search engine correctly ranks results using a multi-factor scoring system. 
The primary ranking factor is the \textbf{TF/IDF score} (relevance of terms). In cases where 
documents have identical TF/IDF scores, the \textbf{PageRank score} (authority of the page) must be 
used as the tie-breaker to determine the final order.

\subsection*{Preconditions}
\begin{itemize}
  \item The index contains at least three documents:
  \begin{itemize}
    \item \textbf{Doc A:} High frequency of query terms ("cat dog"), Low PageRank.
    \item \textbf{Doc B:} Medium frequency of query terms, High PageRank.
    \item \textbf{Doc C:} Low frequency (only "cat"), High PageRank.
  \end{itemize}
  \item PageRank scores are pre-calculated and stored in the \texttt{Pages} table.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Search Execution:} Execute a search query for the multi-term query "cat dog".
  \item \textbf{Intercept Results:} Capture the ordered list of document IDs returned by the search engine.
  \item \textbf{Verify TF/IDF Primacy:} Compare the position of Doc A (high TF/IDF) against Doc B (medium TF/IDF).
  \item \textbf{Verify Tie-breaking:} Identify two documents with identical TF/IDF scores and compare their positions based on PageRank.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item \textbf{TF/IDF Priority:} Documents with both search terms and higher term frequency appear first (Doc A 
  Doc B).
  \item \textbf{PageRank Tie-break:} If two documents have identical relevance scores, the one with the higher PageRank value is ranked higher.
  \item \textbf{Relevance Threshold:} Doc C (only containing "cat") appears last, as its total TF/IDF score for the full query is lowest.
  \item \textbf{Order of Precedence:} Result set order matches: \texttt{Doc A -> Doc B -> Doc C}.
\end{itemize}
